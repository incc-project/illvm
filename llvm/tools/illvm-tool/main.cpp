...
class ILLVMToolTask : public ForwardingTask {
private:
  ILLVMToolTask() : ForwardingTask("illvm-tool", "", nullptr) {}

public:
  static ILLVMToolTask *getInstance() {
    static ILLVMToolTask instance;
    return &instance;
  }
};
...
private:
  ...
  explicit HelloTask(ILLVMToolTask *iLLVMToolTask)
      : ExecutingTask("hello", "Print Hello + helloContent", iLLVMToolTask) {
    argNames.emplace_back("helloContent");
  }

public:
  __attribute__((constructor)) static HelloTask *getInstance() {
    static HelloTask instance(ILLVMToolTask::getInstance());
    return &instance;
  }
};
...
private:
  ...
  explicit JsonCheckTask(ILLVMToolTask *iLLVMToolTask)
...
public:
  __attribute__((constructor)) static JsonCheckTask *getInstance() {
    static JsonCheckTask instance(ILLVMToolTask::getInstance());
    return &instance;
  }
};
...
private:
  explicit FuncVTask(ILLVMToolTask *iLLVMToolTask)
      : ForwardingTask("funcv", "FuncV", iLLVMToolTask) {}

public:
  static FuncVTask *getInstance() {
    static FuncVTask instance(ILLVMToolTask::getInstance());
    return &instance;
  }
};
...
int main(const int argc, char **argv) {
  ...
  const Task *task = ILLVMToolTask::getInstance();
